...
import { addMessage, getHistory } from "@/lib/memory";

import { speak, enableSpeak, disableSpeak } from "@/components/Speaker";
...
export default function LearningClient() {
  ...
  // ✅ SAFE: no suspense error (page.tsx wraps it)
  ...
  useEffect(() => {
    ...
    const intro =
      mode === "general"
        ? `Hi ${userName}! Click on mic and ask me anything.`
        : `Hi ${userName}! Click on mic and start learning ${mode}.`;
  ...
  async function handleUser(text: string) {
    ...
    try {
      ...
      // ✅ NULL SAFE
      const aiText =
        res ?? "Sorry, I could not understand. Please try again.";

      addMessage("assistant", aiText);
      setChatList((p) => [...p, { role: "assistant", text: aiText }]);
  ...
  if (!session) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-500 via-purple-400 to-pink-300">
      ...
      <div className="flex max-w-6xl mx-auto mt-6 px-4 gap-5">
        ...
        <section className="flex-1">
          <div className="bg-white/25 backdrop-blur-xl rounded-3xl shadow-2xl p-6 space-y-6">
            ...
            <VoiceEngine active={running} onText={handleUser} />
          </div>
        </section>
      </div>
    </main>
